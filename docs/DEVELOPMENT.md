...
`install.sh` discovers and pins a published Git tag. Root `start.sh` delegates to
`scripts/start.sh`, which selects Python, sets `PYTHONPATH=src`, and runs
`python -m gnome_web_display.launcher`. All Python modules are under
`src/gnome_web_display/`; UI files are in `web/` and media defaults in `config/`.
...
GStreamer capture approach from the previous implementation with safer startup, audio handling, and cleanup.
...
|-- VERSION                    Single runtime version source
|-- install.sh                 Standalone version/menu bootstrap
|-- start.sh / setup.sh        Thin, backwards-compatible entry points
|-- run.sh                     Compatibility alias
|-- src/gnome_web_display/
|   |-- paths.py               Root/web/config paths and VERSION loading
|   |-- settings.py            Validated TOML/environment configuration
|   |-- launcher.py            Diagnostics, network selection, Docker supervision
|   |-- doctor.py              Read-only diagnostics
|   |-- app.py                 aiohttp HTTP/WebSocket/media gateway
|   |-- host.py                Mutter/PipeWire/GStreamer integration
|   |-- security.py            Sessions and login rate limiting
|   `-- input_protocol.py      Remote-input validation/state
|-- web/                       index.html, login.html, demo.html
|-- config/                    config.example.toml, mediamtx.yml
|-- scripts/                   start.sh, setup.sh, verify.sh
|-- tests/                     Portable application + offline installer tests
|-- tools/                     Demo/UI harnesses and manifest generator
|-- docs/                      Detailed guides, validation and release notes
`-- .github/workflows/ci.yml    Automated checks
```

No frontend bundler, CDN assets, external font dependency, or Node package install
is required to run the app. The browser code is kept inline in `web/index.html`.
...
`config/mediamtx.yml` is actually mounted into the container, read-only. v7.1 shipped a
configuration file that its launcher did not mount. The current configuration also disables unused media
...
personal windows first. See [docs/SCREENSHOTS.md](SCREENSHOTS.md).
...
support matrix. The current report is [docs/TESTING.md](TESTING.md).
...
The `LICENSE` included in the supplied repository is retained unchanged. This
refactor does not select a new license or invent a copyright owner. See
[LICENSE-NOTICE.md](LICENSE-NOTICE.md) for provenance and third-party notes.

For installer internals and tag semantics see [INSTALLER.md](INSTALLER.md). Before
publishing, regenerate the manifest and follow [RELEASING.md](RELEASING.md).
