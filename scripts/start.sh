...
for file in VERSION src/gnome_web_display/__init__.py src/gnome_web_display/launcher.py src/gnome_web_display/settings.py src/gnome_web_display/doctor.py src/gnome_web_display/paths.py; do
  [[ -f "$file" ]] || { echo "ERROR: Missing project file: $file. Re-extract the complete release." >&2; exit 2; }
done
export PYTHONPATH="$ROOT/src${PYTHONPATH:+:$PYTHONPATH}"
exec "$PYTHON" -m gnome_web_display.launcher "$@"
